 ...
 */
public class PotraitBehavior2 extends CoordinatorLayout.Behavior<CircleImageView> {
    ...
    public PotraitBehavior2(Context context, AttributeSet attrs) {
        super(context, attrs);
...
//        }
        child.setLayoutParams(lp);
        float childPositionStart = getStatusBarHeight()+(dependency.getHeight()-DisplayUtils.dip2px(context, 35))/2;
        float childPositionEnd = getStatusBarHeight()+DisplayUtils.dip2px(context, 150)-dependency.getHeight()/2 - DisplayUtils.dip2px(context, 70)/2;
        float currentY = (float) (childPositionStart + (childPositionEnd-childPositionStart)*zoom);
        child.setY(currentY-10);
